    ...
    def main(self):
        while self.running:
            ...
            try:
                ...
                # 打印部分图像数据进行检查
                self.get_logger().info(f'RGB Image Data (first 10 elements): {rgb_image.flat[:10]}')
                self.get_logger().info(f'Depth Image Data (first 10 elements): {depth_image.flat[:10]}')
